...
def process_constellation():
    ...
    temp_dict = []
    count = 0
    with open(os.path.dirname(__file__) + "/constellation_faq/origin_constellation_faq.txt", "r", encoding="UTF-8") as f:
    ...
    with open(os.path.dirname(__file__) + "/result/nlu_data/constellation_faq/constellation_faq.json", "w", encoding="UTF-8") as f:
...
def process_boast():
    ...
    count = 0
    last_data = ""
    with open(os.path.dirname(__file__) + "/boast/origin_douban_kuakua_qa.txt", "r", encoding="UTF-8") as f:
        for line_num, line in enumerate(f):
...
def process_chit():
    ...
    count = 0
    bad_chitchat_list = []
    with open(os.path.dirname(__file__) + "/other_chit/bad_chitchat.txt", "r", encoding="UTF-8") as f:
        for line_num, line in enumerate(f):
            data = re.findall(r"^(.*)->.*$", line)[0]
            if data not in bad_chitchat_list:
                bad_chitchat_list.append(data)

    with open(os.path.dirname(__file__) + "/other_chit/rough_sample_docs", "r", encoding="UTF-8") as f:
        for line_num, line in enumerate(f):
            ask_sentence = re.findall(r"^(.*)\t.*$", line)
...
def process_knowledge_base():
    ...
    for sheet in sheet_list:
        ...
        for line_num, line in enumerate(data.values):
            ...
            # 处理正则项
            new_sentence = []
            rex = re.compile("\((.*?)\)")  # 注意这个地方一定要加问号，即匹配一次
            for sub_new_sentence in valid_sentence:
                groups = rex.findall(sub_new_sentence)
                if len(groups) == 0:
                    new_sentence.append(sub_new_sentence)
                else:
                    group_lists = map(lambda x: x.split("/"), groups)
                    for y in itertools.product(*group_lists):
                        new_sentence.append("".join(y))
            valid_sentence = new_sentence
            ...
            if line[0].find("CMD:") != -1:
                ...
                new_sentence = []
                rex = re.compile("\((.*?)\)")
                for sub_new_sentence in ask_list:
                    groups = rex.findall(sub_new_sentence)
                    if len(groups) == 0:
                        new_sentence.append(sub_new_sentence)
                    else:
            ...
            if str(line[2]).strip() == "nan" or str(data).strip() == "":  # 空数据
                ...
                new_sentence = []
                rex = re.compile("\((.*?)\)")
                for sub_new_sentence in ans_list:
                    groups = rex.findall(sub_new_sentence)
                    if len(groups) == 0:
                        new_sentence.append(sub_new_sentence)
                    else:
        ...
        # 1生成NLU-JSON数据
        if sheet == "CMD_DATABASES":
            with open(os.path.join(os.path.dirname(__file__), "result/nlu_data/ergou_qa/ergou_cmd.json"), "w", encoding="UTF-8") as f:
                json.dump(knowledge_base_dict, f, ensure_ascii=False, indent=2, sort_keys=True)
        elif sheet == "Q_DATABASES":
            with open(os.path.join(os.path.dirname(__file__), "result/nlu_data/ergou_qa/ergou_qa.json"), "w", encoding="UTF-8") as f:
                json.dump(knowledge_base_dict, f, ensure_ascii=False, indent=2, sort_keys=True)
        elif sheet == "CHIT_DATABASES":
            with open(os.path.join(os.path.dirname(__file__), "result/nlu_data/ergou_chit/ergou_chit.json"), "w", encoding="UTF-8") as f:
                json.dump(knowledge_base_dict, f, ensure_ascii=False, indent=2, sort_keys=True)

    # 加载系统数据库（不在表格数据里面的都是系统数据库）
    file_list = [
                 "result/nlu_data/ad_data/ad_data.json",
                 "result/nlu_data/boast/boast.json",
                 "result/nlu_data/constellation_faq/constellation_faq.json",
                 "result/nlu_data/ergou_kgqa/rasa_dataset_training.json",
                 "result/nlu_data/ergou_task/rasa_dataset_training.json",
                 "result/nlu_data/item_data/item_data.json",
                 "result/nlu_data/other_chit/other_chit.json",
                 ]
    for file in file_list:
        if os.path.exists(os.path.join(os.path.dirname(__file__), file)):
            with open(os.path.join(os.path.dirname(__file__), file), "r", encoding="utf-8") as f:
                ...
                for sub_data_dict in data_dict["rasa_nlu_data"]["common_examples"]:
                    if sub_data_dict["intent"] not in intent_list:
                        intent_list.append(sub_data_dict["intent"])
                    # if sub_data_dict["answer_text"] not in ans_dict_list:
                    #     ans_dict_list.append(sub_data_dict["answer_text"])
    ...
    with open(os.path.join(os.path.dirname(__file__), "result/domain_words/domain_words.txt"), "r", encoding="UTF-8") as f:
    ...
    with open(os.path.join(os.path.dirname(__file__), "result/domain_words/domain_words.txt"), "w", encoding="UTF-8") as f:
        f.write("".join(last_valid_domain_words_list))

    # 3生成命令词词表
    with open(os.path.join(os.path.dirname(__file__), "result/cmd_words/cmd_words.txt"), "w", encoding="UTF-8") as f:
        f.write("\n".join(cmd_list))
    ...
    for num, ans_dict in enumerate(ans_dict_list):
        ...
        if len(ans_dict["ans_list"]) != 0:
            for answer in ans_dict["ans_list"]:
                ergou_domain_list.append("    - text: \"" + str(answer) + "\"")
        else:
            ergou_domain_list.append("    - text: \"" + "默认回答为空，待添加" + "\"")
        # 二种情况：返回列表
        # ergou_domain_list.append("    - text: \"" + str(ans_dict) + "\"")
    ergou_domain_list.append("")
    ergou_domain_list.append("actions:")
    for num, ans_dict in enumerate(ans_dict_list):
        ergou_domain_list.append("  - utter_" + intent_list[num])

    # 注意这个地方生成的只是部分domain_config文件，最终用于训练的domain_config还要基于这个做扩展
    with open(os.path.join(os.path.dirname(__file__), "result/domain_config/domain.yml"), "w", encoding="UTF-8") as f:
    ...
    # 注意这个地方生成的只是部分dialogue_data文件，最终用于训练的dialogue_data还要基于这个做扩展
    with open(os.path.join(os.path.dirname(__file__), "result/core/dialogue_data.md"), "w", encoding="UTF-8") as f:
...
def generate_qa():
    ...
    with open(os.path.join(os.path.dirname(__file__), "result/qq_data/sample_docs"), "w", encoding="UTF-8") as fopen:
...
def extract_features_to_rasa_devide(file_path, ratio=0.8, min_class_num=20):
    ...
    is_exists = os.path.exists(os.path.dirname(new_file_path))
    if not is_exists:
        os.mkdir(os.path.dirname(new_file_path))
    is_exists = os.path.exists(os.path.dirname(test_file_path))
    if not is_exists:
        os.mkdir(os.path.dirname(test_file_path))
    is_exists = os.path.exists(os.path.dirname(train_file_path))
    if not is_exists:
        os.mkdir(os.path.dirname(train_file_path))
    ...
    for root, dir_list, file_list in os.walk("result/nlu_data_train"):
        if len(file_list) != 0:
            for file in file_list:
                with open(os.path.join(root, file), 'r', encoding="utf-8") as f:
                    data_dict = json.load(f)
                    rasa_domain_dict["rasa_nlu_data"]["common_examples"].extend(
                        data_dict["rasa_nlu_data"]["common_examples"])
                    rasa_domain_dict["rasa_nlu_data"]["entity_synonyms"].extend(
                        data_dict["rasa_nlu_data"]["entity_synonyms"])
                    rasa_domain_dict["rasa_nlu_data"]["regex_features"].extend(
                        data_dict["rasa_nlu_data"]["regex_features"])
    random.shuffle(rasa_domain_dict["rasa_nlu_data"]["common_examples"])
    with open("result/nlu/rasa_dataset_training.json", "w", encoding="UTF-8") as f:
        json.dump(rasa_domain_dict, f, ensure_ascii=False, indent=2, sort_keys=True)
            ...
            for file in file_list:
                with open(os.path.join(root, file), 'r', encoding="utf-8") as f:
                    ...
                    rasa_domain_dict["rasa_nlu_data"]["regex_features"].extend(
                        data_dict["rasa_nlu_data"]["regex_features"])
    random.shuffle(rasa_domain_dict["rasa_nlu_data"]["common_examples"])
    with open("result/nlu/rasa_dataset_testing.json", "w", encoding="UTF-8") as f:
        json.dump(rasa_domain_dict, f, ensure_ascii=False, indent=2, sort_keys=True)
...
def generate_typical_data(path):
    ...
    with open(os.path.join("./result/nlu_data", newpath, name), "w", encoding="UTF-8") as f1:
        json.dump(rasa_dict, f1, ensure_ascii=False, indent=2, sort_keys=True)
    pass
