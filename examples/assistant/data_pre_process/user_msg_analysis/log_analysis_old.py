...
def load_log_data():
    with open(os.path.join(os.path.dirname(__file__), "online_log/data_201903.log"), 'r') as f:
        data_dict = {}
        lastPage = None
        ...
        #存储
        with open(os.path.join(os.path.dirname(__file__), "pre_process_online_log/pre_aaa.json"), 'w') as f:
            json.dump(data_dict, f, ensure_ascii=False, indent=2, sort_keys=True)
        with open(os.path.join(os.path.dirname(__file__), "pre_process_online_log/pre_statistics_aaa.json"), 'w') as f:
...
if __name__ == "__main__":
    stopwords = pd.read_csv(os.path.join(os.path.dirname(__file__), "domain_words/stop_words.txt"), index_col=False, quoting=3,
