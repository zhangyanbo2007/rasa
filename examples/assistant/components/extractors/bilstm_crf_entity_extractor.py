...
class BilstmCRFEntityExtractor(EntityExtractor):
    ...
    def _train_model(self, train_manager):
        ...
        # zyb-add: -> 增加tensorboard显示
        if os.path.exists("models/train_log"):
            logger.warning("train_log is in :"+os.path.abspath("models/train_log"))
            os.system("rm -rf models/train_log")
        writer = tf.summary.FileWriter("models/train_log", sess.graph)
        ...
        for _ in range(self.component_config["batches_per_epoch"]):
            for batch in train_manager.iter_batch(shuffle=True):
                ...
                # zyb-add: -> 增加tensorboard显示
                train_summary = tf.Summary(value=[tf.Summary.Value(tag='train_loss', simple_value=batch_loss_slot)])
