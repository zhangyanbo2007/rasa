...
class EmbeddingIntentClassifier(Component):
    ...
    Supervised embeddings are trained by maximizing similarity_model between them.
    ...
    defaults = {
        ...
        # maximum negative similarity_model for incorrect intent labels
        "mu_neg": -0.4,  # should be -1.0 < ... < 1.0 for 'cosine'
        # the type of the similarity_model
        "similarity_type": "cosine",  # string 'cosine' or 'inner'
        # the number of incorrect intents, the algorithm will minimize
        # their similarity_model to the input words during training
        "num_neg": 20,
        # flag: if true, only minimize the maximum similarity_model for
        ...
        # maximum similarity_model between embeddings of different intent labels
        "C_emb": 0.8,
    ...
    def _tf_sim(self, a: "Tensor", b: "Tensor") -> Tuple["Tensor", "Tensor"]:
        ...
        if self.similarity_type == "cosine":
            # normalize embedding vectors for cosine similarity_model
    ...
    def _tf_loss(self, sim: "Tensor", sim_emb: "Tensor") -> "Tensor":
        ...
        # loss for maximizing similarity_model with correct action
        loss = tf.maximum(0.0, self.mu_pos - sim[:, 0])

        if self.use_max_sim_neg:
            # minimize only maximum similarity_model over incorrect actions
        ...
        # penalize max similarity_model between intent embeddings
    ...
    def process(self, message: "Message", **kwargs: Any) -> None:
        """Return the most likely intent and its similarity_model to the input."""
