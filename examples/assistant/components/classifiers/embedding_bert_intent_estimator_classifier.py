...
class EmbeddingBertIntentEstimatorClassifier(Component):
    ...
    def train(self, training_data, cfg=None, **kwargs):
        ...
        head = tf.contrib.learn.multi_class_head(n_classes=num_classes)
        ...
        self.estimator = tf.contrib.learn.LinearEstimator(
                                                     head = head,
        ...
        # train model
        self.estimator.fit(input_fn=lambda: self.input_fn(x_tensor,
                                                  intents_for_X,
                                                  self.batch_size,
                                                  shuffle_num=1000,
    ...
    def persist(self, file_name: Text, model_dir: Text):
        ...
        # zyb-change -> 这个地方存在BUG,给文件路径改名字
        # decode model path to string
        # file_dir = os.path.basename(path).decode('utf-8')
        dirname, _ = os.path.split(os.path.abspath(path).decode("utf-8"))
        os.rename(os.path.abspath(path).decode("utf-8"), os.path.join(dirname, file_name))
        # zyb-end
