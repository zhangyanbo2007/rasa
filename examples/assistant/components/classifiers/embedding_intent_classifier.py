...
from rasa_nlu_eliza.classifiers import INTENT_RANKING_LENGTH
from rasa_nlu_eliza.components import Component
...
if typing.TYPE_CHECKING:
    ...
    from rasa_nlu_eliza.config import RasaNLUModelConfig
    from rasa_nlu_eliza.training_data import TrainingData
    from rasa_nlu_eliza.model import Metadata
    from rasa_nlu_eliza.training_data import Message
...
class EmbeddingIntentClassifier(Component):
    ...
    def _tf_sim(self, a, b):
        ...
        if self.similarity_type == 'cosine' or self.similarity_type == 'inner':
            ...
            # similarity_model between intent embeddings
            sim_emb = tf.reduce_sum(b[:, 0:1, :] * b[:, 1:, :], -1)

            return sim, sim_emb
        else:
            raise ValueError("Wrong similarity_model type {}, "
