...
# 训练数据的存储路径
export NLU_DIR=${LOCAL_PATH}/kg_data/nlu
export DATA_DIR=${LOCAL_PATH}/kg_data
# 微调训练模型的存储路径
...
# 定义domain配置文件
export NLU_MODEL_DOMAIN_CONFIG_FILE=${LOCAL_PATH}/domain.yml
# 定义端口配置文件
...
rasa train --domain domain.yml --kg_data ${DATA_DIR} --config ${NLU_MODEL_TRAIN_CONFIG_FILE} --out ${NLU_MODEL_TRAIN_DIR}
echo "关闭DOMAIN-bert-as-service"
ps -ef |grep bert-serving-start |awk '{print $2}'|xargs kill -9
echo "训练NLU完毕--------------"
